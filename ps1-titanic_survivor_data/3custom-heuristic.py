...
def custom_heuristic(file_path):
    ...
    dummy1 = pandas.get_dummies(df['Pclass'], prefix = 'pclass')
    dummy2 = pandas.get_dummies(df['Sex'], prefix = 'sex')
    cols_to_keep = ['Survived', 'Age', 'SibSp']

    data = df[cols_to_keep].join(dummy1.ix[:,'pclass_2':])
    data = data.join(dummy2.ix[:,'sex_male':])
    data['intercept'] = 1.0

    train_cols = data.columns[1:]

    logit = sm.Logit(data['Survived'],
            data[train_cols],
            missing = 'drop')

    result = logit.fit(disp = 0)

    pred = logit.predict(result.params, data[train_cols])
    pred[~numpy.isnan(pred)] = numpy.around(pred[~numpy.isnan(pred)])

    df['pred_survived'] = pred

    for passenger_index, passenger in df.iterrows():
        passenger_id = passenger['PassengerId']
        if numpy.isnan(passenger['pred_survived']):
            if passenger['Sex'] == 'female':
                predictions[passenger_id] = 1.0
            else:
                predictions[passenger_id] = 0
        else:
            predictions[passenger_id] = passenger['pred_survived']
